            ...
            <TabsList className="flex overflow-x-auto whitespace-nowrap scrollbar-hide snap-x justify-start h-auto p-2 bg-muted/50 w-full sm:flex-wrap mb-8">
              {servicesData.map((category) => (
                <TabsTrigger
                  key={category.name}
                  value={category.name}
                  className="px-8 py-4 text-base font-semibold data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary data-[state=active]:to-accent data-[state=active]:text-primary-foreground rounded-full shadow-md data-[state=active]:shadow-lg transition-all duration-300 hover:scale-105"
                >
                  {category.name}
                </TabsTrigger>
              ))}
